/**
 * screen_script.c - Script Detail Screen Implementation
 */
...
script_screen_t* script_screen_create(SDL_Renderer *renderer,
    ...
    script_screen_t *screen = calloc(1, sizeof(script_screen_t));
...
int script_screen_set_entity(script_screen_t *screen, const char *entity_id) {
    ...
    if (!screen->entity) {
        strcpy(screen->status_message, "Script not found");
        return 0;
    }
...
int script_screen_handle_input(script_screen_t *screen, SDL_Event *event) {
    ...
    if (input_button_pressed(BTN_A)) {
        if (run_script(screen)) {
            strcpy(screen->status_message, "Running!");
        } else {
            strcpy(screen->status_message, "Run failed");
        }
        return 0;
    }
...
void script_screen_render(script_screen_t *screen) {
    ...
    ui_draw_header(r, font_header, font_small, "SCRIPT", is_online);

    if (!screen->entity) {
        ui_draw_text(r, font_body, "No script selected", 320, 200, COLOR_TEXT_SECONDARY, TEXT_ALIGN_CENTER);
        return;
    }
    ...
    // Icon
    icons_draw(screen->icons, "script_code", 320 - 32, 120, 64);

    // State
    char state_text[64];
    snprintf(state_text, sizeof(state_text), "State: %s", screen->entity->state);
    ui_draw_text(r, font_body, state_text, 320, 200, COLOR_TEXT_PRIMARY, TEXT_ALIGN_CENTER);
    ...
    ui_draw_text(r, font_body, "[A] RUN", 320, 347, COLOR_GB_DARKEST, TEXT_ALIGN_CENTER);
    ...
    const char *hints[] = {"[A] Run", "[Y] Fav", "[B] Back"};
...
static void parse_script_info(script_screen_t *screen) {
    ...
    if (triggered_ptr) {
        ...
        if (strncmp(triggered_ptr, "null", 4) != 0) {
            ...
            format_timestamp(iso_time, screen->last_triggered, sizeof(screen->last_triggered), "Last run");
        } else {
            strcpy(screen->last_triggered, "Never run");
        }
    }
...
static int run_script(script_screen_t *screen) {
    ...
    ha_response_t *response = ha_client_call_service(
        *screen->client_ptr, "script", "turn_on", screen->entity->entity_id, NULL
    );
